...
struct block_device {
	sector_t		bd_start_sect;			//块设备的起始扇区
	sector_t		bd_nr_sectors;	   	    //块设备的总扇区数
	struct gendisk *	bd_disk;		 	//对应的块设备
	struct request_queue *	bd_queue;	    //io操作请求队列
//	struct disk_stats __percpu *bd_stats;   //块设备的性能统计信息
 	unsigned long		bd_stamp;			//块设备的时间戳
	atomic_t		__bd_flags;	            // partition number + flags
	dev_t			bd_dev;
	struct address_space	*bd_mapping;	/* page cache */
	atomic_t		bd_openers;
	spinlock_t		bd_size_lock; 
	void *			bd_claiming; //求块设备的持有者和正在申请者的指针。
	void *			bd_holder;
//	const struct blk_holder_ops *bd_holder_ops;//指向操作持有者的操作结构的指针
	struct mutex		bd_holder_lock;
	int			bd_holders;
//	struct kobject		*bd_holder_dir;
	atomic_t		bd_fsfreeze_count; /* number of freeze requests */
	struct mutex		bd_fsfreeze_mutex; /* serialize freeze/thaw */
	struct partition_meta_info *bd_meta_info;  //指向分区元数据的指针
	int			bd_writers;
...
static sector_t bdev_nr_sectors(struct block_device *bdev){
