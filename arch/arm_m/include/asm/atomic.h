 ...
 * Modified for uClinux on STM32F407
 */
...
/*
 * On ARMv7-M, ordinary assignment (str instruction) doesn't clear the local
 ...
 *
 * For STM32F407 (Cortex-M4, ARMv7-M), these instructions are available.
 */
...
{                                       \
    ...
    int result;                                 \
                                        \
    /* prefetchw(&v->counter); - prefetch not available */ \
...
static inline int arch_atomic_cmpxchg_relaxed(atomic_t *ptr, int old, int new)
{
    ...
    /* prefetchw(&ptr->counter); - prefetch not available */
...
static inline int arch_atomic_fetch_add_unless(atomic_t *v, int a, int u)
{
    ...
    /* smp_mb(); - Memory barriers might need specific implementation */
    /* prefetchw(&v->counter); - prefetch not available */
