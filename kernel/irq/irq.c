...
void clean_interrupt_flag(uint32_t flag);
...
void C_IRQ_Dispatcher(int irq_number)       
{
    ...
    if(Run_IRQ_Fn(IRQ_list[irq_number]) == 0){
        printk(KERN_WARNING "unregister interrpute:(%d) On core:%d\n",irq_number,__get_task_using_cpu_core());
        clean_interrupt_flag(irq_number);
    }
}
