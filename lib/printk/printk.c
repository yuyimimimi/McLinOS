...
static struct file* tty_device = NULL;
static struct spinlock printk_spinlock;

int printk_service_init(void) {
    tty_device = filp_open(CONFIG_PRINTTK_USE_TTY_DEVICE, O_RDWR, 0);
    if (IS_ERR(tty_device)) {
        printk("tty device:%s open failed,use early printk to out put continue\n",CONFIG_PRINTTK_USE_TTY_DEVICE);
        tty_device = NULL;
        return -1;
    }
...
static int printk_log_level_threshold = 7;  
static int printk_show_log_level      = 1;  
